...
describe("graphic-gate", () => {
  ...
  it("flags horizontal luma bands as axis-heavy", () => {
    const w = 64;
    const h = 64;
    const data = new Uint8Array(w * h * 4);
    for (let y = 0; y < h; y += 1) {
    ...
    const analysis = analyzePixels(data, w, h);
    assert.equal(analysis.axisHeavy, true);
    assert.ok(analysis.axisOrient >= 1.8);
  });

  it("does not flag isotropic photographic noise as axis-heavy", () => {
    const w = 128;
    const h = 128;
    const data = new Uint8Array(w * h * 4);
    let seed = 1;
    ...
    const analysis = analyzePixels(data, w, h);
    assert.equal(analysis.axisHeavy, false);
  });
