...
describe("fuse", () => {
  ...
  it("lifts an axis-heavy mid-range visual without inventing a verdict from orientation alone", () => {
    const lifted = fuseScores({
      visual: 0.56,
      provenance: { ai: false, camera: false },
      graphic: { isGraphic: false, axisHeavy: true },
    });
    assert.ok(lifted.fusedBeforeCalibration > 0.56);
    assert.ok(lifted.reasons.includes("axis-edge"));

    const orientOnly = fuseScores({
      visual: 0.2,
      provenance: { ai: false, camera: false },
      graphic: { isGraphic: false, axisHeavy: true },
    });
    assert.ok(Math.abs(orientOnly.fusedBeforeCalibration - 0.2) < 1e-9);
  });
