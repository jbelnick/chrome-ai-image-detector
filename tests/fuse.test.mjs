...
describe("fuse", () => {
  ...
  it("drops a hue-peaked mid-high visual without inventing a real verdict from hue alone", () => {
    const dropped = fuseScores({
      visual: 0.7,
      provenance: { ai: false, camera: false },
      graphic: { isGraphic: false, huePeakish: true },
    });
    assert.ok(dropped.fusedBeforeCalibration < 0.7);
    assert.ok(dropped.reasons.includes("hue-peak"));

    const hueOnly = fuseScores({
      visual: 0.2,
      provenance: { ai: false, camera: false },
      graphic: { isGraphic: false, huePeakish: true },
    });
    assert.ok(Math.abs(hueOnly.fusedBeforeCalibration - 0.2) < 1e-9);
  });
