...
export function fuseScores({
  ...
  const evenSatLift = config.evenSatLift ?? 0;
  if (
    evenSatLift > 0 &&
    graphic?.evenSat &&
    score >= (config.evenSatBandMin ?? 0.5) &&
    score < (config.evenSatBandMax ?? 0.68)
  ) {
    score = Math.min(0.92, score + evenSatLift);
    reasons.push("even-sat");
  }
