...
export function fuseScores({
  ...
  const isolateLift = config.isolateLift ?? 0;
  if (
    isolateLift > 0 &&
    graphic?.isolated &&
    score >= (config.isolateBandMin ?? 0.5) &&
    score < (config.isolateBandMax ?? 0.68)
  ) {
    score = Math.min(0.92, score + isolateLift);
    reasons.push("center-surround");
  }
