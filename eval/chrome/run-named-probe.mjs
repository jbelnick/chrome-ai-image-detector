#!/usr/bin/env node
/**
 * Chrome-path dump for the named film / UI probe list.
 *
 * Same offscreen fusion as the extension: createImageBitmap +
 * OffscreenCanvas + shipped fuse. Sharp CF-crop does not count.
 *
 *   GRAIN_NAMED_PROBE_DIR=/path/to/probes npm run eval:chrome:named-probe
 */
...
export const REQUIRED_PROBE_NAMES = [
...
export function dumpPathFromRoot(repo = root) {
  return join(repo, "eval/chrome/named-probe-chrome.json");
}

async function main() {
  const data = process.env.GRAIN_NAMED_PROBE_DIR || join(root, "eval/data/named-probe");
  const scored = await listProbe(data);
  const missing = REQUIRED_PROBE_NAMES.filter((name) => !scored.some((row) => row.name === name));
  if (missing.length) {
    throw new Error(`named-probe dir ${data} missing ${missing.join(", ")}`);
  }
  ...
  const profile = join(tmpdir(), `grain-named-probe-${process.pid}`);
  const child = spawn(
    chrome,
    [
      "--headless=new",
  ...
  try {
    ...
    console.log("");
    console.log("Grain chrome-path named-probe dump (same offscreen fusion as the extension)");
    ...
    await writeFile(
      ...
      JSON.stringify(
        {
          path: "chrome-ort-web-named-probe",
          sharp: false,
          ...
          n: rows.length,
          rows,
          note: "Chrome-path dump. createImageBitmap + OffscreenCanvas + shipped fuse. Sharp does not count.",
        },
        null,
        2,
      ),
