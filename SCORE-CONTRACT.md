...
3. `FUSE_DEFAULTS` nips: leftover 358-mix band drops/lifts are frozen junk. Allowed named product rules: `bias = 0`, provenance short-circuit, `muted-scan` / Charlesworth (do not revert without live-page evidence), `compressedThumb` canvas resample (nearest CF upsample when source short-edge < 440; graphic flags still read the medium crop). Do not add mix nips.
